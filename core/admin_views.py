...
def admin_dashboard(request):
    ...
    for khatma in Khatma.objects.filter(created_at__gte=thirty_days_ago).order_by('-created_at')[:10]:
        recent_activities.append({
            ...
            'url': f'/admin/core/khatma/{khatma.id}/change/'
        })
    ...
    for assignment in PartAssignment.objects.filter(
        ...
        recent_activities.append({
            ...
            'url': f'/admin/core/partassignment/{assignment.id}/change/'
        })
    
    # Recent chat messages
    for chat in KhatmaChat.objects.all().order_by('-created_at')[:5]:
        recent_activities.append({
            'activity_type': f'رسالة في ختمة {chat.khatma.title}',
            'user': chat.user.username,
            'timestamp': chat.created_at,
            'url': f'/admin/core/khatmachat/{chat.id}/change/'
        })
