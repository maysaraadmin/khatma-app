...
class ProfileModelTest(TestCase):
    ...
    def test_get_family_members(self):
        ...
        # Create a family admin profile
        family_admin = Profile.objects.create(
            user=User.objects.create_user(username='familyadmin', password='testpassword'),
            account_type='family',
            family_admin=True
        )
        
        # Create family member profiles
        family_member1 = Profile.objects.create(
            user=User.objects.create_user(username='member1', password='testpassword'),
            account_type='family',
            family_admin=False,
            family_group=family_admin
        )
        
        family_member2 = Profile.objects.create(
            user=User.objects.create_user(username='member2', password='testpassword'),
            account_type='family',
            family_admin=False,
            family_group=family_admin
        )
        
        # Test that family admin can get family members
        ...
        # Test that non-family admin gets empty queryset
...
class KhatmaModelTest(TestCase):
    ...
    def test_get_progress_percentage(self):
        ...
        # Create KhatmaParts for the khatma
        for i in range(1, 31):
            part = QuranPart.objects.get(part_number=i)
            part_assignment = PartAssignment.objects.create(
                khatma=self.khatma,
                part=part,
                is_completed=(i <= 15)  # Mark first 15 parts as completed
            )
        
        # Expected progress: 15/30 = 50%
