...
'\n'
from core.forms import DeceasedForm, KhatmaCreationForm, PartAssignmentForm, UserProfileForm, UserProfileEditForm
...
class DeceasedFormTest(TestCase):
    ...
    def setUp(self):
        '''"""Function to setUp."""'''
        self.valid_data = {'name': 'Test Deceased', 'death_date': timezone.now().date(), 'account_type': 'individual', 'relation': 'Parent', 'memorial_day': True, 'memorial_frequency': 'yearly'}
    ...
    def test_deceased_form_with_image(self):
        ...
        form = DeceasedForm(data=form_data, files={'photo': self.image_file})
        self.assertTrue(form.is_valid())
...
class KhatmaCreationFormTest(TestCase):
    ...
    def setUp(self):
        '''"""Function to setUp."""'''
        self.valid_data = {'title': 'Test Khatma', 'khatma_type': 'regular', 'description': 'Test description', 'is_public': True, 'frequency': 'once', 'visibility': 'public', 'send_reminders': True, 'reminder_frequency': 'weekly'}
...
class PartAssignmentFormTest(TestCase):
    ...
    def setUp(self):
        ...
        self.part_assignment = PartAssignment.objects.create(khatma=self.khatma, part=self.part)
        self.valid_data = {'notes': 'Test notes', 'dua': 'Test dua'}

    def test_part_assignment_form_valid(self):
        """Test that the form is valid with valid data"""
        form = PartAssignmentForm(data=self.valid_data, instance=self.part_assignment)
        self.assertTrue(form.is_valid())
...
class UserProfileFormTest(TestCase):
    ...
    def setUp(self):
        ...
        self.valid_data = {'username': 'testuser2', 'email': 'test2@example.com', 'first_name': 'Test2', 'last_name': 'User2'}
