...
class KhatmaPartInline(admin.TabularInline):
    '''"""Class representing KhatmaPartInline."""'''
    model = KhatmaPart
    extra = 0
    readonly_fields = ['part_number']

class ParticipantInline(admin.TabularInline):
    '''"""Class representing ParticipantInline."""'''
    model = Participant
    extra = 0

@admin.register(Khatma)
class KhatmaAdmin(admin.ModelAdmin):
    '''"""Class representing KhatmaAdmin."""'''
    ...
    fieldsets = (('Basic Information', {'fields': ('title', 'creator', 'description', 'khatma_type', 'frequency')}), ('Group Settings', {'fields': ('group', 'is_group_khatma', 'auto_distribute_parts')}), ('Memorial Settings', {'fields': ('deceased', 'memorial_prayer', 'memorial_image')}), ('Social Features', {'fields': ('is_public', 'visibility', 'allow_comments', 'social_media_hashtags', 'social_media_image')}), ('Status', {'fields': ('is_completed', 'target_completion_date', 'completed_at', 'start_date', 'end_date')}), ('Sharing and Participants', {'fields': ('sharing_link', 'max_participants')}), ('Reminders', {'fields': ('send_reminders', 'reminder_frequency')}), ('Timestamps', {'fields': ('created_at',)}))

@admin.register(Deceased)
class DeceasedAdmin(admin.ModelAdmin):
    '''"""Class representing DeceasedAdmin."""'''
    ...
    fieldsets = (('Basic Information', {'fields': ('name', 'death_date', 'birth_date', 'photo', 'biography', 'added_by')}), ('Additional Information', {'fields': ('relation', 'cause_of_death', 'burial_place')}), ('Memorial Settings', {'fields': ('memorial_day', 'memorial_frequency')}), ('Timestamps', {'fields': ('created_at',)}))

@admin.register(KhatmaPart)
class KhatmaPartAdmin(admin.ModelAdmin):
    '''"""Class representing KhatmaPartAdmin."""'''
...
class ParticipantAdmin(admin.ModelAdmin):
    '''"""Class representing ParticipantAdmin."""'''
...
class PartAssignmentAdmin(admin.ModelAdmin):
    '''"""Class representing PartAssignmentAdmin."""'''
...
class QuranReadingAdmin(admin.ModelAdmin):
    '''"""Class representing QuranReadingAdmin."""'''
