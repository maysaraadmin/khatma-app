...
class KhatmaEditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'target_completion_date': forms.DateInput(attrs={'type': 'date'}),
            'description': forms.Textarea(attrs={'rows': 4}),
            'memorial_prayer': forms.Textarea(attrs={'rows': 3}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user and self.instance.khatma_type == 'memorial':
            self.fields['deceased'] = forms.ModelChoiceField(
                ...
                label='المتوفى',
                widget=forms.Select(attrs={'class': 'form-control'}),
                initial=self.instance.deceased
            )
...
class KhatmaShareForm(forms.Form):
    ...
    email_addresses = forms.CharField(
        ...
        required=False,
        label='عناوين البريد الإلكتروني (مفصولة بفواصل)'
    )
    ...
    share_on_social = forms.BooleanField(
        required=False,
        initial=True,
        label='مشاركة على وسائل التواصل الاجتماعي'
    )
...
class KhatmaFilterForm(forms.Form):
    ...
    khatma_type = forms.ChoiceField(
        choices=[('', 'جميع الأنواع')] + list(Khatma.KHATMA_TYPE_CHOICES),
        required=False,
        label='نوع الختمة'
    )
