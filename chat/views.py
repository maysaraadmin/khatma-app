...
def khatma_chat(request, khatma_id):
    ...
    if request.method == 'POST':
        ...
        notifications = [
            Notification(user=participant.user, notification_type='khatma_chat', message=f'{request.user.email} sent a new message in khatma {khatma.title}', related_khatma=khatma, related_user=request.user)
...
def group_chat(request, group_id):
    ...
    if request.method == 'POST':
        ...
        notifications = [
            Notification(user=membership.user, notification_type='group_chat', message=f'{request.user.email} sent a new message in group {group.name}', related_group=group, related_user=request.user)
